...
from pist.game.entities import EntityConfigStore
from pist.loenn import LoennPlacement, LoennRegistry, LoennWarning
from pist.ui.entity_rules import (
    RULE_CONFLICTS_ID,
    EntityRuleApp,
    LoennPlacementItem,
    search_placements,
...
""",
    ...
    async def check() -> None:
        app = EntityRuleApp(LoennRegistry([placement]), EntityConfigStore(config_path))
    ...
    asyncio.run(check())
    rules = EntityConfigStore(config_path).load()
...
""",
    ...
    async def check() -> None:
        app = EntityRuleApp(
            LoennRegistry(), EntityConfigStore(shared_path=shared_path, local_path=local_path)
        )
...
def test_rule_app_keeps_current_stylesheet_when_refresh_fails(tmp_path, monkeypatch) -> None:
    ...
    async def check() -> None:
        app = TestRuleApp(LoennRegistry(), EntityConfigStore(config_path))
...
def test_rule_app_filters_template_list_after_text_changes(tmp_path) -> None:
    ...
    async def check() -> None:
        app = EntityRuleApp(LoennRegistry([strawberry, unrelated]), EntityConfigStore(config_path))
        async with app.run_test() as pilot:
