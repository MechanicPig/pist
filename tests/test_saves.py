...
def test_sid_for_map_file_removes_b_and_c_side_suffixes() -> None:
    ...
    with pytest.raises(ValueError, match='Not a map file path'):
        sid_for_map_file('Content/Maps/0-Intro.bin')
...
def test_save_reader_uses_native_stats_and_falls_back_to_mod_save_data(tmp_path: Path) -> None:
    ...
    map_stats = save.get_map_stats(LocalMap(file_path='Maps/Author/Pack/Map.bin', dialog_key='Map'))
    ...
    assert save.is_in_progress(LocalMap(file_path='Maps/Author/Pack/Map.bin', dialog_key='Map'))
...
def test_save_reader_reads_collab_return_to_lobby_sessions(tmp_path: Path) -> None:
    ...
    assert save.is_in_progress(
        LocalMap(file_path='Maps/Collab/1-Maps/Unfinished.bin', dialog_key='Unfinished')
    )
    assert not save.is_in_progress(
        LocalMap(file_path='Maps/Collab/1-Maps/Finished.bin', dialog_key='Finished')
    )
...
def test_save_reader_preserves_numeric_like_collab_session_sid(tmp_path: Path) -> None:
    ...
    assert save.is_in_progress(LocalMap(file_path='Maps/01.bin', dialog_key='Numeric_SID'))
...
def test_save_reader_prefers_everest_current_session(tmp_path: Path) -> None:
    ...
    assert not save.is_in_progress(
        LocalMap(file_path='Maps/Vanilla/Map.bin', dialog_key='Vanilla_Map')
    )
    assert save.is_in_progress(
        LocalMap(file_path='Maps/Mod/Map-B.bin', dialog_key='Mod_Map', side='B')
    )
