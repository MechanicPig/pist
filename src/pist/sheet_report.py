"""Human-readable coverage reports and draft-field metadata from Sheet inspections."""
...
from pist.models import InspectionReport, SmartSheetField
...
FIELD_SOURCES = {
    'Mod元数据名': ('当前草稿', 'everest.yaml'),
    'Mod名': ('计划自动', 'GameBanana 标题和链接'),
    '地图名': ('当前草稿', 'Dialog 本地化名称'),
    ...
    '用时': ('当前草稿', '原生存档 TimePlayed'),
    '死亡数': ('当前草稿', '原生存档 Deaths'),
    '红草莓数': ('计划自动', '.bin 地图实体分析'),
    '月莓数': ('计划自动', '.bin 地图实体分析'),
    '磁带': ('计划自动', '.bin 地图实体分析'),
    '水晶之心': ('计划自动', '.bin 地图实体分析'),
    '主房间数': ('当前草稿', '用户保存的地图主路线房间数'),
    '状态': ('计划自动', '由通关和收集情况推导'),
    '标签': ('人工填写', '尚无可靠自动提取规则'),
    '体感难度': ('人工填写', '主观评价'),
    '难度子阶': ('人工填写', '主观评价'),
...
MAIN_TABLE_TITLE = '主表'
MANUAL_DRAFT_FIELD_TITLES = frozenset(
    {
        '体感难度',
        '难度子阶',
        '标注难度',
...
def manual_draft_fields(report: InspectionReport) -> tuple[ManualDraftField, ...]:
    """Return supported optional draft fields declared by the inspected main table."""
    for inspection in report.sheets:
        ...
        result: list[ManualDraftField] = []
        for raw_field, field in zip(raw_fields, fields, strict=True):
            if field.field_title not in MANUAL_DRAFT_FIELD_TITLES:
                continue
            options: tuple[str, ...] = ()
            ...
            result.append(ManualDraftField(field.field_title, field.field_type, options))
        return tuple(result)
    return ()
